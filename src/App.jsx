...
const WORLD_ITEMS = [
  ...
  { label: "活動",       angle:  95,  dist: 0.85, color: "rgba(251,191,36,.9)"  },
  { label: "人との繋がり", angle: -95, dist: 0.75, color: "rgba(196,181,253,.9)" },
  { label: "ストーリー", angle: 135,  dist: 0.9,  color: "rgba(249,168,212,.9)" },
]
...
const EXPERIENCE_CATALOG = [
  ...
  { id: 2, ic: "👗", title: "バーチャル着せかえ", desc: "服装提案・着せ替え・クローゼット連携によるコーデ提案体験。あなたに似合うスタイルをAIが提案。", tags: ["ファッション","コーデ","スタイル"], url: "https://virtual-tryon-app-phi.vercel.app/",  ready: true },
...
function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const onDown = () => outerRef.current?.classList.add("cursor--press")
    const onUp   = () => outerRef.current?.classList.remove("cursor--press")
    const onOver = e => {
      const interactive = !!e.target.closest("a, button, [role=button], .exp-card, .wv-chip")
      outerRef.current?.classList.toggle("cursor--link", interactive)
    }
    ...
    let raf
    const tick = () => {
      lagged.current.x += (mouse.current.x - lagged.current.x) * 0.11
      lagged.current.y += (mouse.current.y - lagged.current.y) * 0.11
      if (outerRef.current) {
        outerRef.current.style.left = `${lagged.current.x}px`
        outerRef.current.style.top  = `${lagged.current.y}px`
      }
      if (innerRef.current) {
...
function AuroraBg() {
  ...
  return (
    <div className="aurora-root" aria-hidden="true">
      <div ref={orbsRef} className="aurora-orbs">
...
// ── World-view visualization with click-glow ──────────────────────────
      ...
      <div className="worldview-center">
        ...
        {WORLD_ITEMS.map((item, i) => {
          ...
          return (
            <div
              key={i}
              className="wv-chip-pos"
              style={{ "--cx": `${x}px`, "--cy": `${y}px`, "--delay": `${i * 0.5}s` }}
            >
              <div
                className={cn("wv-chip", isLit && "wv-chip--lit")}
                style={{
                  color:       item.color,
                  borderColor: isLit ? item.color.replace(".9)", ".75)") : item.color.replace(".9)", ".32)"),
                  background:  isLit ? item.color.replace(".9)", ".18)") : item.color.replace(".9)", ".07)"),
                  "--glow-c":  item.color.replace(".9)", ".5)"),
                }}
                onClick={() => toggle(i)}
                role="button"
                tabIndex={-1}
...
// ── Scroll-triggered Step item ────────────────────────────────────────

function StepItem({ s, index, isLast }) {
  const [ref, visible] = useInView(0.3)
  return (
    <div
      ref={ref}
      className={cn("step-item", visible && "step-visible")}
      style={{ "--delay": `${index * 0.12}s` }}
    >
      <div className="step-left">
        <div className="step-num" style={{ color: s.color, borderColor: s.color.replace(".9)", ".28)") }}>
          STEP<br />{s.step}
        </div>
        {!isLast && (
          <div
            className="step-line"
            style={{ "--line-color": s.color.replace(".9)", ".55)") }}
          />
        )}
      </div>
      <div className="step-card" style={{ borderColor: s.color.replace(".9)", ".16)") }}>
...
export default function App() {
  ...
  const filteredNews   = newsFilter === "すべて" ? NEWS_ITEMS : NEWS_ITEMS.filter(n => n.cat === newsFilter)
  ...
  return (
    <div className="app-root">
      ...
      <section className="section" id="news" ref={newsRef}>
        <div className={cn("sec-inner", newsInView && "sec-visible")}>
          <div className="sec-eyebrow">AI News</div>
          <h2 className="sec-title"><span className="title-hand">最新AIニュース</span></h2>
          <p className="sec-desc">AIの最前線を毎日更新。いち早くキャッチして、先手を打とう。</p>

          <div className="news-filter-wrap">
            {newsCategories.map(cat => {
              const c   = NEWS_CAT_COLORS[cat]
              const on  = newsFilter === cat
              return (
                <button key={cat}
                  className={cn("news-filter-chip", on && "on")}
                  style={on && c ? { color: c[0], background: c[1], borderColor: c[2] } : {}}
          ...
          <div className="news-grid">
            {filteredNews.slice(0, newsVisible).map((n, i) => {
              const c = NEWS_CAT_COLORS[n.cat] || ["rgba(255,255,255,.5)", "rgba(255,255,255,.06)", "rgba(255,255,255,.15)"]
              return (
                <div key={i} className="news-card" style={{ "--delay": `${i * 0.07}s` }}>
                  <div className="news-card-top">
                    <span className="news-cat-badge" style={{ color: c[0], background: c[1], borderColor: c[2] }}>{n.cat}</span>
          ...
          {newsVisible < filteredNews.length && (
            <div className="more-wrap">
              <button className="more-btn" onClick={() => setNewsVisible(v => v + 4)}>もっと見る</button>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="section" id="experience" ref={expRef}>
        <div className={cn("sec-inner", expInView && "sec-visible")}>
          <div className="sec-eyebrow">Experience Catalog</div>
          <h2 className="sec-title"><span className="title-hand">AI体験カタログ</span></h2>
          ...
          <div className="exp-grid">
            {EXPERIENCE_CATALOG.map((item, i) => (
              ...
              >
                <div className="exp-card-rainbow" aria-hidden="true" />
                ...
                <div className="exp-tags">
                  {item.tags.map((t, j) => (
                    <span key={t} className="exp-tag" style={{ color: TAG_COLORS[j % TAG_COLORS.length] }}>{t}</span>
                  ))}
                </div>
      ...
      {expModal && (
        <div className="modal-overlay" onClick={() => setExpModal(null)}>
          <div className="modal-box modal-box--exp" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {expModal.ready && expModal.url ? (
                ...
                <div className="modal-soon"><span>🚧</span> この体験は現在準備中です</div>
              )}
            </div>
          </div>
        </div>
      ...
      <section className="section" id="worries" ref={worriesRef}>
        <div className={cn("sec-inner", worriesInView && "sec-visible")}>
          <div className="sec-eyebrow">Your Worries</div>
          <h2 className="sec-title"><span className="title-hand">こんなお悩みありませんか？</span></h2>
          <div className="worries-grid">
            {WORRIES.map((w, i) => (
              <div key={i} className="worry-card" style={{ "--delay": `${i * 0.09}s` }}>
          ...
          <div className="sec-eyebrow" style={{ marginTop: "56px" }}>What AI Can Do</div>
          <h2 className="sec-title"><span className="title-hand">AIで出来ること</span></h2>
          ...
          <div className="whatai-grid">
            {[
              ...
              <div key={i} className="whatai-card" style={{ "--delay": `${i * 0.07}s` }}>
      ...
      <section className="section steps-section" id="steps">
        <div className="sec-inner">
          ...
          <h2 className="sec-title"><span className="title-hand">AI収益化5STEP</span></h2>
          <p className="sec-desc">世界観の可視化から継続収益まで、5つのステップで仕組みを構築します。</p>
          <div className="steps-timeline">
            {FIVE_STEPS.map((s, i) => (
              <StepItem key={s.step} s={s} index={i} isLast={i === FIVE_STEPS.length - 1} />
            ))}
          </div>
          <div className="steps-cta">
            <button className="btn-main" onClick={() => scrollTo("consult")}>まずは無料相談する →</button>
          </div>
        </div>
      </section>
      ...
      <section className="section" id="services" ref={svcRef}>
        <div className={cn("sec-inner", svcInView && "sec-visible")}>
          <div className="sec-eyebrow">AI Services</div>
          <h2 className="sec-title"><span className="title-hand">AIサービス紹介</span></h2>
          ...
          <div className="svc-grid">
            {AI_SERVICES.map((svc, i) => (
              <a key={svc.name} href={svc.url} target="_blank" rel="noopener noreferrer"
                className={cn("svc-card", svc.color)} style={{ "--delay": `${i * 0.05}s` }}>
      ...
      <section className="section" id="categories" ref={catRef}>
        <div className={cn("sec-inner", catInView && "sec-visible")}>
          <div className="sec-eyebrow">Categories</div>
          <h2 className="sec-title"><span className="title-hand">AIカテゴリ</span></h2>
          <p className="sec-desc">ジャンルではなく、あなたの目的から探す。</p>
          <div className="cat-grid">
            {AI_CATEGORIES.map((c, i) => (
              <div key={c.name} className="cat-card" style={{ "--delay": `${i * 0.08}s` }}>
      ...
      {/* ⑧ 無料相談 ─────────────────────────────────────────────── */}
      <section className="section" id="consult" ref={consultRef}>
        <div className={cn("sec-inner", consultInView && "sec-visible")}>
          <div className="sec-eyebrow">Free Consultation</div>
          <h2 className="sec-title"><span className="title-hand">無料相談</span></h2>
          <p className="sec-desc">AIの活用・世界観づくり・収益化についてお気軽にご相談ください。<br />2営業日以内にご返信します。</p>
      ...
      <section className="cta-section" id="cta" ref={ctaRef}>
        <AuroraBg />
        <div className={cn("cta-inner", ctaInView && "sec-visible")}>
